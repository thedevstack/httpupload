...
class XMPP extends XMLStream {
	...
	public function message_handler($xml) {
		...
		$payload['xml'] = $xml;
		$this->log->log("Message: {$payload['body']}", XMPPLog::LEVEL_DEBUG);
	...
	public function presence_handler($xml) {
		...
		$this->log->log("Presence: {$payload['from']} [{$payload['show']}] {$payload['status']}",  XMPPLog::LEVEL_DEBUG);
	...
	protected function sasl_failure_handler($xml) {
		$this->log->log("Auth failed!",  XMPPLog::LEVEL_ERROR);
		$this->disconnect();
		
		throw new XMPPException('Auth failed!');
	}
	/**
